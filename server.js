...
// تم تعديلها لتجنب خطأ "Cannot find module credentials.json" على Vercel
async function getDoc() {
    let credsData;
    try {
        if (process.env.GOOGLE_CREDS) {
            // استخدام البيانات من متغيرات البيئة في Vercel
            credsData = JSON.parse(process.env.GOOGLE_CREDS);
        } else {
            // البحث عن الملف محلياً فقط في حالة التطوير (Development)
            const credsFilePath = path.join(__dirname, 'credentials.json');
            if (fs.existsSync(credsFilePath)) {
                credsData = require(credsFilePath);
            } else {
                throw new Error("Missing GOOGLE_CREDS environment variable or credentials.json file");
            }
        }
        ...
        await doc.useServiceAccountAuth({
            ...
            // معالجة مفتاح الـ private_key ليعمل بشكل صحيح في جميع البيئات
...
app.listen(PORT, () => {
    ...
    📍 Mode: Cloud Production (Vercel Ready)
    📡 Port: ${PORT}
    =======================================================
    `);
});
